...
- [x] NPC 对话系统（分支选项）/ NPC dialogue with branching options
- [x] 任务系统（击杀 / 到达区域 / BOSS，任务日志）/ quest system (kill / reach / boss, quest log)
- [x] 多区域地图切换（新手村 → 草原 → 沙漠 → 雪山）/ multi-zone map switching
- [x] 存档系统（自动 / 手动，继续游戏）/ save system (auto & manual, continue)
- [x] 转职系统（Lv.10 二选一，属性加成生效）/ class advancement with real stat bonuses
- [x] 商店系统（金币掉落 + 商人购买）/ shop system (gold drops + merchant)
- [x] BOSS 战（每区域 BOSS + 血条 + 通关结局）/ boss fights with health bar & victory ending
- [x] 音频系统（Web Audio 程序化音效 + 区域 BGM）/ audio system (procedural SFX + per-zone BGM)

### 🚧 后续计划 / Future Ideas

- 装备强化 / 镶嵌系统 / equipment enhancement & socketing
- 更多职业、技能与法术 / more classes, skills and spells
- NPC 剧情任务链扩展 / extended NPC story quest chains
- 无尽模式与排行榜 / endless mode & leaderboards
